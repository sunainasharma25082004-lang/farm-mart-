...
export const AddressScreen = ({ navigation }) => {
  const { savedAddresses, selectedAddress, setSelectedAddress, addAddress, removeAddress } = useApp();
  ...
  const handleSaveNewAddress = () => {
    if (!newAddress.fullName || !newAddress.phone || !newAddress.addressString) {
      alert("Please fill in all details");
      return;
    }
    addAddress(newAddress);
    setModalVisible(false);
    setNewAddress({ label: 'Home', fullName: '', phone: '', addressString: '' });
  };
        ...
        {savedAddresses.map((addr) => {
          ...
          return (
            ...
            >
              ...
              <TouchableOpacity onPress={() => removeAddress(addr.id)} style={styles.deleteBtn}>
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.labelRow}>
              {['Home', 'Work', 'Other'].map(lbl => (
                <TouchableOpacity
                  key={lbl}
                  style={[styles.labelChip, newAddress.label === lbl && styles.labelChipActive]}
                  onPress={() => setNewAddress({ ...newAddress, label: lbl })}
                >
                  <Text style={[styles.labelChipText, newAddress.label === lbl && styles.labelChipTextActive]}>{lbl}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <TextInput
              ...
              onChangeText={t => setNewAddress({ ...newAddress, addressString: t })}
            />
